...
from tools.glob import GLOB_TOOL_DEFINITION, GlobToolInput, execute_glob_tool
from tools.grep import GREP_TOOL_DEFINITION, GrepToolInput, execute_grep_tool
...
class TestExecuteGlobTool:
    ...
    def test_no_matches(self, test_dir):
        with pytest.raises(ValueError, match="No files"):
            execute_glob_tool(GlobToolInput(pattern="*.xyz", path=test_dir))

    def test_directory_not_found(self):
        with pytest.raises(ValueError, match="[Pp]ath not found"):
            execute_glob_tool(GlobToolInput(pattern="*.ts", path="/no/such/dir"))
...
class TestExecuteGrepTool:
    ...
    def test_invalid_regex(self, test_dir):
        with pytest.raises(ValueError, match="[Ii]nvalid regex"):
            execute_grep_tool(GrepToolInput(pattern="[invalid", path=test_dir))
